...
class DistanceFinder2:
	...
	def findSquareMarker(image):
		...
		return markerDim, edged

	@staticmethod
	def draw(image, boundingBox, dist, color=(0, 255, 0), thickness=2):
		# draw a bounding box around the marker and display the distance to the marker on the image
		(x, y, w, h) = boundingBox
		cv2.rectangle(image, (x, y), (x + w, y + h), color, thickness)
		cv2.putText(image, "%.2f mts" % dist, (image.shape[1] - 250, image.shape[0] - 20),
