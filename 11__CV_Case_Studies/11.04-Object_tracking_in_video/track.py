...
# if a video path was not supplied, select 0 and grab the reference to the webcam
camera = cv2.VideoCapture(0)
#camera = cv2.VideoCapture("11 - CV Case Studies/11.04-Object tracking in video/BallTracking_01.mp4")
...
while True:
	...
	for (lower, upper, colorName) in colorRanges:
		...
		if len(cnts) > 0:
			...
			# only draw the enclosing circle and text if the radious meets a minimum size
			if radius > 10:
